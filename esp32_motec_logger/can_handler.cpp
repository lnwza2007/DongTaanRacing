...
float can_handler_get_value(size_t index) {
    ...
    float val = NAN;
    if (xSemaphoreTake(state_mutex, pdMS_TO_TICKS(5)) == pdTRUE) {
        // Return NAN if data is stale (no update for > 1000 ms)
        // but do NOT wipe the stored value so it recovers if CAN resumes
        if (channel_timestamps[index] != 0 && (millis() - channel_timestamps[index]) <= 1000) {
